...
function createContentTS(counter) {
    ...
    var positions = getUniqueRandomXYPositionsTS(counter + 1);
    var imgs = [];
    for (var _i = 0, _a = positions.slice(0, positions.length - 1); _i < _a.length; _i++) {
        var position = _a[_i];
        //////debugger   
        ...
        img.addEventListener('click', function () {
            ...
            if (element) {
                ...
                var xPosition_1 = Math.floor(Number(Number(img.style.left.split("px")[0]) - window.innerWidth / 2));
                //console.log(img.style.left.split("px")[0] - window.innerWidth / 2);
                var element_1 = document.elementFromPoint(xPosition_1, Math.floor(Number(Number(img.style.top.split("px")[0]))));
                if (element_1) {
                    //console.log('loser22');
                }
                else {
...
function getUniqueRandomXYPositionsTS(numberOfPositions) {
    var positions = [];
    for (var i = 0; i < numberOfPositions; i++) {
        var position = getRandomXYPositionTS();
...
function getRandomXYPositionTS() {
    ...
    var randomX = Math.abs(Math.floor(Math.random() * screenWidth / 2));
    var randomY = Math.abs(Math.floor(Math.random() * screenHeight));
    return {
        x: randomX,
        y: randomY
    };
